...
class DKVMN(Module):
    def __init__(self, num_c, dim_s, size_m, dropout=0.2, emb_type='qid', emb_path="", pretrain_dim=768):
        ...
        self.e_layer = Linear(self.dim_s + BERT_EMB_DIM, self.dim_s)
        self.a_layer = Linear(self.dim_s + BERT_EMB_DIM, self.dim_s)

    def forward(self, q, r, id, emb_dic, question, qtest=False):
        ...
        combined_emb = torch.cat([v, bert_embeddings], dim=-1)  
        ...
        # Write Process
        e = torch.sigmoid(self.e_layer(combined_emb))
        a = torch.tanh(self.a_layer(combined_emb))
